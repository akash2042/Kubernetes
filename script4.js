function fun1(){
    ...
    xhr.open("POST","http://192.168.1.22/cgi-bin/main.py?cmd=" + "sudo kubectl create deploy " + d_name + " --replicas=" + replica + " --image=" + i_name + " -n " + ns_name, false);
    xhr.send();
...
function fun3(){
    ...
    var ns_name = i.value;
    var xhr= new XMLHttpRequest();
    xhr.open("POST","http://192.168.1.22/cgi-bin/main.py?cmd=" + "sudo kubectl get deployment --namespace " + ns_name, false);
    xhr.send();
    var output = xhr.responseText;
    var outputbox = document.getElementById("output");
    outputbox.style.color = "green";
