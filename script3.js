function fun1(){
    ...
    xhr.open("POST","http://192.168.1.22/cgi-bin/main.py?cmd=" + "sudo kubectl run " + i + " --image=" + j + " -n " + l, false);
    xhr.send();
...
function fun3(){
    ...
    var j = i.value;
    var xhr= new XMLHttpRequest();
    xhr.open("POST","http://192.168.1.22/cgi-bin/main.py?cmd=" + "sudo kubectl get pods --namespace " + j, false);
    xhr.send();
    var output = xhr.responseText;
    var outputbox = document.getElementById("output");
    outputbox.style.color = "green";
