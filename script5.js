...
function fun1(){
    ...
    xhr.open("POST","http://192.168.1.22/cgi-bin/main.py?cmd=" + "sudo kubectl get deployments", false);
    xhr.send();
...
function fun2(){
    ...
    var l = k.value;
    var xhr= new XMLHttpRequest();
    xhr.open("POST","http://192.168.1.22/cgi-bin/main.py?cmd=" + "sudo kubectl expose deployment " + i + " --port=" + j + "--type=NodePort -n " + l, false);
    xhr.send();
    var output = xhr.responseText;
    var outputbox = document.getElementById("output");
    outputbox.style.color = "green";
